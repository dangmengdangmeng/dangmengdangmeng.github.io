<template>
  <div class="color_f6 w_h100">
    <div class="bg_white">
      <div
        class="top_nav w_90 m_center b_b size16 flex_space_between ptb5 "
        style="border-color:#000;"
      >
        <div>作业列表</div>
        <div @click="goTo">全部作业</div>
      </div>
    </div>
    <list-item :list_item="workerList.cellsData"></list-item>
    <page-bottom-button :paramsList="buttonList"></page-bottom-button>
  </div>
</template>
<script type="text/javascript">
  import ListItem from '@/components/list_item.vue'
  import PageBottomButton from '@/components/page_bottom_button.vue'
  export default {
    name: '',
    components: { ListItem, PageBottomButton },
    data () {
      return {
        buttonList: [
          {
            type: 'weui-btn_primary',
            text: '新建报告',
            action: () => {
              wx.showActionSheet({
                itemList: ['上线工作报告',  '非上线工作报告'],
                success (res) {
                  if (res.tapIndex === 0) {
                    wx.navigateTo({url: '/pages/worker/report-reports/newOnlineReport/main'})
                  } else if (res.tapIndex === 1) {
                    wx.navigateTo({url: '/pages/worker/report-reports/newOfflineReport/main'})
                  }
                }
              })
            }
          }
        ],
        workerList: {
          cellsData: [
            {
              title: '01月12日作业',
              type: '跟车作业',
              time: '2018-11-29 16:47:22',
              status: '未完成',
              'status_color': '#f00',              
              'plan_text': '非',
              'plan_color': '#008000',
              link: '/pages/worker/report-reports/reportDetails/main?id=1',
              id: 1
            },
            {
              title: '01月12日作业',
              type: '调查作业',
              time: '2018-11-29 16:47:22',
              status: '未完成',
              'status_color': '#f00',
              'plan_text': '非',
              'plan_color': '#008000',
              link: '/pages/worker/report-reports/reportDetails/main?id=2',
              id: 2
            },
            {
              title: '01月12日作业',
              type: '途中',
              time: '2018-11-29 16:47:22',
              status: '已完成',
              'status_color': '#00f',
              'plan_text': '上',
              'plan_color': '#199ed8',
              link: '/pages/worker/report-reports/reportDetails/main?id=3',
              id: 3
            },
            {
              title: '01月12日作业',
              type: '宾馆滞留',
              time: '2018-11-29 16:47:22',
              status: '已完成',
              'status_color': '#00f',
              'plan_text': '上',
              'plan_color': '#199ed8',
              link: '/pages/worker/report-reports/reportDetails/main?id=4',
              id: 4
            }
          ],
        }
      }
    },
    props: [],
    computed: {},
    methods: {
      goTo () {
        wx.navigateTo({url:'/pages/worker/report-reports/allReports/main'})
      }
    }
  }
</script>
<style>
</style>