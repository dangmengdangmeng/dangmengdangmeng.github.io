...
import { ROOM_STATE, Logger } from '@global/roomConstants';
...
export const sendSignalling = ({
  ...
  const { roomStatus } = store.getState().classroom;
  if (roomStatus !== ROOM_STATE.CONNECTED) {
    return;
  }
  let signalData = '';
  ...
  const msgId = signalId || id || name;
  if (isDelMsg) {
    YsGlobal.roomClient.delMsg(name, msgId, toId);
  } else {
    const extraData = {
      type,
      write2DB,
    ...
    YsGlobal.roomClient.pubMsg(name, msgId, toId, signalData, isSave, associatedMsgID, associatedUserID, extraData);
  }
};
  ...
  if (message) {
    ...
    if (typeof messageTemp === 'string') {
      try {
        ...
        Logger.error('toJsonParse:data is not json string!');
      }
    }
    let textMsg;
    ...
    YsGlobal.roomClient.sendChatMsg(textMsg, toId || '__all', { ...messageTemp, ...{ isToSender } });
  }
};
...
export const setUserProperty = (id, properties, toId = '__all') => {
  ...
  YsGlobal.roomClient.setProperty(id, properties, toId);
};
