...
/* 根据文件后缀名判断属于什么文件类型 */
export const getFileType = filesuffix => {
  let filetype;
...
export const getDeviceTypeDetail = () => {
  const devices = ['WindowPC', 'MacPC', 'AndroidPhone', 'AndroidPad', 'iPhone', 'iPad', 'WindowClient', 'MacClient', 'AndroidTV', 'WXApplet', 'MobileBrowser'];
  const sUserAgent = navigator.userAgent;
  const findDevices = devices.filter(it => sUserAgent.includes(it));
  ...
  if (detectOS().includes('Win')) {
    return 'WindowPC';
  }
  if (findDevices && findDevices.length) {
    return findDevices[0];
  }
  return 'WindowPC';
};
...
export const wrapFileInfo = (file, isDel = false) => {
  const isDynamicPPT = file.fileprop === 1 || file.fileprop === 2;
  const isH5Document = file.fileprop === 3;
  const isGeneralFile = !isDynamicPPT && !isH5Document;
  const isMediaFile = /(mp3|mp4|webm)/g.test(file.filetype);
  const fileInfoObj = {
    isDel,
    isGeneralFile,
    isMedia: isMediaFile,
    ...
    isH5Document,
    action: '',
    mediaType: isMediaFile ? file.filetype : '',
    filedata: {
      fileid: file.fileid,
      currpage: 1,
      ...
      pptstep: 0,
      steptotal: 0,
      filecategory: file.filecategory !== undefined ? Number(file.filecategory) : 0, // 0:课堂 ， 1：系统
      fileprop: Number(file.fileprop),
      isContentDocument:
        file.isContentDocument !== undefined && file.isContentDocument !== null && typeof Number(file.isContentDocument) === 'number'
          ? Number(file.isContentDocument)
          : 0,
      dynamicppt: isDynamicPPT ? 1 : 0,
    },
  };
  return fileInfoObj;
};
