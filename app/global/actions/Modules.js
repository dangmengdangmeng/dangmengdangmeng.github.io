...
const moduleNames = {
  ...
  countdown: {
    status: SET_COUNTDOWNSTATUS,
    data: SET_COUNTDOWNDATA,
  },
  chat: {
    status: SET_CHATSTATUS,
    data: SET_CHATDATA,
  },
};
...
const changeResponserStatus = data => {
  store.dispatch({
    type: SET_RESPONSER_STATUS,
    payload: data,
  });
}

const changeContestStudentList = data => {
  store.dispatch({
    type: SET_CONTEST_STUDENT,
    payload: data,
  });
}
...
export default {
  ...
  applyWheatState,
  isplayMicro,
  changeResponserStatus,
  changeContestStudentList,
};
