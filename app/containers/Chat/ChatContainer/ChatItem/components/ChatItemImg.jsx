...
function ChatItemImg(props) {
  ...
  const sendTs = new Date(contents.sendTs);
  const sendTime = `${sendTs.getHours() < 9 ? `0${sendTs.getHours()}` : sendTs.getHours()}:${
    sendTs.getMinutes() < 9 ? `0${sendTs.getMinutes()}` : sendTs.getMinutes()
  }`;
  // 点击名字把用户信息传上去
  ...
  const Regs = /([^\*]+)(.png|.gif|.jpg|.sendTimejpeg|.jpeg)$/;
  ...
  return (
    <div className={`${contents.isMe ? 'isme' : ''} ChatItemImg`}>
      <p className="user-title" onClick={headerNameClick}>
        <span className="send-time">{sendTime}</span>·<span className="username">{contents.who}</span>
      </p>
      <p className="chat-img-warp">
        {isLoad ? null : <img className="maskImg_load" alt="" />}
        {isLoad && isError ? <img className="maskImg_load_error" alt="" /> : null}
        <img
          className="maskImg"
          src={imgurl}
          alt="聊天图片"
