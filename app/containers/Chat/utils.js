...
export function getNowDate() {
  const timeNow = new Date().getHours() >= 12 ? '下午' : '上午';
  return `${timeNow}${new Date().getHours() < 10 ? `0${new Date().getHours()}` : new Date().getHours()} : ${
...
export function numToTime(num, format = 'YYYY-MM-DD HH-mm-ss') {
  let _num = num;
  if (typeof _num !== 'number') return '';
  if (`${_num}`.length === 10) _num *= 1000;
