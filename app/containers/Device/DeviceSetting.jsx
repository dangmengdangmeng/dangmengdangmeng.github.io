...
class DeviceSetting extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      isVideoMirror: JSON.parse(localStorage.getItem('isVideoMirror')) === null ? true : JSON.parse(localStorage.getItem('isVideoMirror')),
      isFirst: false,
      temporaryUseDevices: {},
    };
  ...
  componentDidMount() {
    ...
    if (!isLiveRoom || SessionRoom.getMyself().role !== 2 || JSON.parse(localStorage.getItem('firstTimeWheat')) || this.props.isCheckMore) {
      this.setState({
        isFirst: true,
      });
    }
  ...
  okButtonOnClick = () => {
    ...
    // 修改强制镜像用户属性
    UserService.setVideoMirror(this.state.isVideoMirror);
  ...
  stopDevice() {
    DeviceManager.stopCameraTest();
    DeviceManager.stopSpeakerTest();
    DeviceManager.stopMicrophoneTest();
  }
    ...
    return (
      ...
      >
        <div className="device_header">
          ...
          <Icon type="close" onClick={this.closeButtonOnClick} />
        </div>
