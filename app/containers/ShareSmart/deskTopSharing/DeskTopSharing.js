...
export default class DeskTopSharing extends Component {
  ...
  componentDidMount() {
    Event.on('screen-state-changed', this.screenStateChanged.bind(this), this.listernerBackupid);
    Event.on('signal-reconnect', this.signalReconnect.bind(this), this.listernerBackupid);
  ...
  userPropertiesUpdate = data => {
    ...
    if (id === mySelf.id && properties.candraw !== undefined) {
      if (!properties.candraw && isPlayFlag && isMe) {
        SessionRoom.stopShareScreen();
      }
    }
  };
  ...
  screenStateChanged(data) {
    ...
    if (published) {
      ...
      if (userId !== SessionRoom.getMyself().id) {
        this.setState({
          isPlayFlag: true,
          screenStream: { streamUserId: userId },
          isMe: false,
        });
        SessionRoom.playVideo(userId, `screen${userId}`, { type: 'screen', fit: 'contain' });
      } else {
        this.setState({
          isPlayFlag: true,
          screenStream: undefined,
      ...
      this.setState({
        isPlayFlag: false,
        screenStream: undefined,
        isMe: userId === SessionRoom.getMyself().id,
      });
      // SendElectronMsg.hideDesktopMask();
    }
    Actions.setScreenShare({
      hasShare: [0, 1].includes(SessionRoom.getMyself().role) || userId === SessionRoom.getMyself().id ? hasShare : false,
      userId,
    });
  }
