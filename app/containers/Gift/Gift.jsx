...
import './Gift.scss';
const music = require('./music/gift_default.mp3');
...
export const giftPush = (users = []) => {
  ...
  ReactDOM.render(
    userList.map(user => {
      const videoEle = document.getElementById(`baseVideoBox_${user.id}`);
      ...
      return (
        <React.Fragment key={user.id}>
          ...
          <Gift key={user.id} className={animationObj.className} />
        </React.Fragment>
      );
    }),
    giftContainer,
  );
  giftContainer.querySelector('audio').volume = 0.8;
  setTimeout(() => {
