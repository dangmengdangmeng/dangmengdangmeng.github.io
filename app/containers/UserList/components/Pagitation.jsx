...
const Pagitation = props => {
  ...
  return (
    <div className="paging-container">
      <span className={`paging-left icon-left ${currentPage <= 1 ? 'end-page' : ''}`} onClick={handlerClickPrev}></span>
      <div className="paging-container-info">
        <span className="paging-sum">{currentPage}</span>/<span className="paging-sum">{pageSum || 1}</span>
      </div>
      <span className={`paging-right icon-right ${currentPage >= pageSum ? 'end-page' : ''}`} onClick={handlerClickNext}></span>
    </div>
  );
};
